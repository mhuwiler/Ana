#!/usr/bin/env python3
"""AK4 jet kinematics, event-level variables, and b-tagging plots — with data overlay."""
...
MAX_DATA_FILES = 1     # 0 = all data files
MAX_EVENTS     = None  # None = default (100000 from samples.yaml)
CUTS           = []    # no cuts (data doesn't have COI/UParT columns)
TRIGGERS       = ["NoTrigger", "DST_JetHT"]  # data v1 only has JetHT trigger branch
...
PLOT_VARS = [
    ...
    # PlotVar("nFatJets",   r"Number of AK8 fat jets",          8,    0,     8),  # not in data v1
    # Global event
    ...
    # # 4-jet  # not in data v1 (uses v2 branches for 4-vector)
    # PlotVar("m4j",       r"$m_{4j}$ (leading 4 jets) [GeV]", 32,   0,   800),
    # # b-tagged jets # This vairbales are nto present in the data yet, so we will add them later
...
if __name__ == '__main__':
    ...
    ctx = setup(__file__, samples=SAMPLES, triggers=TRIGGERS, cuts=CUTS, data=DATA, nthreads=NTHREADS, nplot_workers=NPLOT_WORKERS,
                theme=THEME, overwrite=OVERWRITE, max_mc_files=MAX_MC_FILES,
                max_data_files=MAX_DATA_FILES, max_events=MAX_EVENTS)
