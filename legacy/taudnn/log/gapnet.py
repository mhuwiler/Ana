...
def placeholder_inputs(batch_size, num_point, num_features):
    ...
    pid_pl = tf.placeholder(tf.int32, shape=(batch_size))
    return pointclouds_pl, pid_pl
...
def get_model(point_cloud, is_training, num_class,params, 
    ...
    net = tf_util.fully_connected(net, params[10], bn=True, is_training=is_training,
                                  activation_fn=tf.nn.relu,
                                  scope=scname+'fc1',bn_decay=bn_decay)
    net = tf_util.dropout(net, keep_prob=0.6, is_training=is_training,
                       scope=scname+'dp1')

    net = tf_util.fully_connected(net, params[11], bn=True, is_training=is_training,
                                  activation_fn=tf.nn.relu,
...
def get_loss(pred, label,num_class=2):
  """ pred: B,NUM_CLASSES
      label: B, """
