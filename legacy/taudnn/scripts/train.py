...
parser.add_argument('--num_point', type=int, default=20, help='Point Number  [default: 100]')
parser.add_argument('--max_epoch', type=int, default=200, help='Epoch to run [default: 200]')
parser.add_argument('--batch_size', type=int, default=10, help='Batch Size during training [default: 64]')
...
parser.add_argument('--data_dir', default='/Users/mhuwiler/cernbox/DoctoralThesis/Analysis/data/v10', help='directory with data [default: hdf5_data]')
parser.add_argument('--nfeat', type=int, default=13, help='Number of features PF [default: 16]')
parser.add_argument('--ncat', type=int, default=3, help='Number of categories [default: 2]')
parser.add_argument('--year', type=int, default=21, help='Year')
...
if FLAGS.year == 17:    
    TRAIN_FILE = os.path.join(H5_DIR, 'train_TAU_UL17.h5')
    TEST_FILE = os.path.join(H5_DIR, 'test_TAU_UL17.h5')
elif FLAGS.year == 18:    
    TRAIN_FILE = os.path.join(H5_DIR, 'train_TAU_UL18.h5')
    TEST_FILE = os.path.join(H5_DIR, 'test_TAU_UL18.h5')
else: 
    TRAIN_FILE = os.path.join(H5_DIR, 'train_TAU.h5')
    TEST_FILE = os.path.join(H5_DIR, 'test_TAU.h5')
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            loss = MODEL.get_loss(pred_pl, labels_pl)
...
def train_one_epoch(sess, ops, train_writer):
    ...
    current_data, current_label= provider.load_h5(TRAIN_FILE,'seg')
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    current_data, current_label = provider.load_h5(TEST_FILE,'seg')
    current_data, current_label, _ = provider.shuffle_data(current_data,current_label)
    ...
    for batch_idx in range(num_batches):
        ...
        if len(y_pred)==0:
            ...
            y_lab=np.concatenate((y_lab,batch_label==1),axis=0)



    fpr, tpr, thresholds = metrics.roc_curve(y_lab.flatten(), y_pred.flatten(), pos_label=1)    
